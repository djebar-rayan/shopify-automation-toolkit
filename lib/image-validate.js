...
// ============================================================
// lib/image-validate.js — Validation d'une image générée
// ------------------------------------------------------------
// Garde-fou avant upload :
//   - taille du fichier ≥ 50 KB
//   - dimensions ≥ 800×800 (si lisibles dans le PNG/JPEG)
// ============================================================
...
function validateGeneratedImage(base64, mimeType) {
  ...
  if (buf.length < MIN_SIZE_KB * 1024) {
    return {
      valid: false,
      reason: `Trop petite (${Math.round(buf.length / 1024)} KB < ${MIN_SIZE_KB} KB)`,
    };
  }
  let w = 0, h = 0;
  ...
  if (w > 0 && h > 0 && (w < MIN_DIM || h < MIN_DIM)) {
    return { valid: false, reason: `Résolution trop basse (${w}×${h})` };
  }
