...
// ============================================================
// lib/gemini-text.js — Text generation through Gemini Flash Lite
// ------------------------------------------------------------
// Model used: config.GEMINI_TEXT_MODEL
// Automatic retry on 429/503 (60s wait) and other errors (2s).
// ============================================================
...
function callGeminiText(prompt, opts = {}) {
  if (!config.GEMINI_API_KEY || !config.GEMINI_API_KEY.startsWith('AIza')) {
    return Promise.reject(new Error('GEMINI_API_KEY invalid or missing (must start with AIza).'));
  }
