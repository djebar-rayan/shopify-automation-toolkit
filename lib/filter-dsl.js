...
// ============================================================
// lib/filter-dsl.js — Mini-DSL de filtre sur blocs store-data
// ------------------------------------------------------------
// Syntaxe acceptée (combinable par virgule = AND) :
...
function applyFilter(blocks, filterStr) {
  ...
  if (!f || /^tous?(\s+les\s+\w+)?$/.test(f)) return blocks;
...
function testClause(b, c) {
  ...
  if (/^seo_title\s+manquant$/.test(c)) return b.seoTitleMissing;
  if (/^seo_description\s+manquant$/.test(c)) return b.seoDescMissing;
  ...
  console.warn(`  ⚠️  Filtre inconnu ignoré : « ${c} »`);
  return true;
}
